...
#load data
load("ProcessedData/m1.1_muri.Rdata")
load("ProcessedData/detect_data_list_muri.Rdata")
...
for (k in c(1:6)) {
  ...
  m1.1terms <- if (all(c("utm.lat", "utm.lon") %in% env_terms)) {
    
    other_terms <- env_terms[
      !env_terms %in% c("utm.lat", "utm.lon")
    ]
...
save(m1.1_AUC_df, m1.1_TSS_df, m1.1_TSS_plot, file = "ProcessedData/m1.1_performance_muri.Rdata")
