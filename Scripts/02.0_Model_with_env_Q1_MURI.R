...
detect_per_species <- detect_data_merge_muri %>% 
  ...
  group_by(BestTaxon) %>% 
  summarize(nDetect = sum(Detected), groups = "drop") %>%
  st_drop_geometry()
...
detect_data_list <- split(detect_data_merge_muri, detect_data_merge_muri$BestTaxon)
...
save(detect_data_list, file = "ProcessedData/detect_data_list_muri.Rdata")
...
for (i in 1:length(detect_data_list)){
  m1.0_temp <- bam(Detected ~ ti(utm.lon, utm.lat, bs = "tp"),
                   family = "binomial",
                   method = "fREML",
...
for (i in 1:length(detect_data_list)){
  m1.0_pred_grid <- expand_grid(utm.lon = seq(min(detect_data_list[[i]]$utm.lon, na.rm = TRUE),
                                                   max(detect_data_list[[i]]$utm.lon, na.rm = TRUE),
                                                   by = 10000),
                                utm.lat = seq(min(detect_data_list[[i]]$utm.lat, na.rm = TRUE),
                                                   max(detect_data_list[[i]]$utm.lat, na.rm = TRUE),
                                                   by = 10000))
...
save(m1.0list, m1.0summary, m1.0sePreds_list, file = "ProcessedData/m1.0_muri.Rdata")
...
save(m1.1_model_selection, m1.1_selected_models, 
     ...
     Q1_model_compare, file = "ProcessedData/m1.1_muri.Rdata")
